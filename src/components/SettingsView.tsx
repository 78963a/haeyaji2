...
export function SettingsView({ settings, tasks, onSaveSettings, onImportData }: SettingsViewProps) {
  ...
  const handleExecuteRestore = () => {
    ...
    if (success) {
      setModalDialog({
        type: 'alert',
        title: '🎉 복구 완료',
        message: '백업 데이터가 성공적으로 완벽 복구되었습니다!',
      ...
      setModalDialog({
        type: 'alert',
        title: '⚠️ 복구 실패',
        message: '데이터 포맷이 잘못되었습니다. 유효한 해야지 백업 JSON 파일인지 확인해주세요.',
        onConfirm: () => setModalDialog(null)
      });
    }
  ...
  const handleCreateCategory = (e: React.FormEvent) => {
    ...
    const savedLabel = newCategoryLabel.trim();
    setNewCategoryLabel('');
    ...
    setModalDialog({
      type: 'alert',
      title: '🏷️ 카테고리 추가 완료',
      message: `'${savedLabel}' 태그 카테고리가 새로 소환되었습니다.`,
      onConfirm: () => setModalDialog(null)
    });
  };
  ...
  const handleDeleteCategory = (catId: string, label: string) => {
    setModalDialog({
      type: 'confirm',
      title: '🗑️ 카테고리 영구 삭제',
      message: `'${label}' 카테고리를 영구 삭제하시겠습니까?\n해당 태깅 분류를 사용 중인 일들의 태그도 자동으로 무효화됩니다.`,
      onConfirm: () => {
  ...
  return (
    <div className="space-y-6 pb-24">
      ...
      <div className="bg-white border-4 border-black p-5 shadow-[6px_6px_0px_0px_#000]">
        ...
        <p className="text-sm text-zinc-700 leading-relaxed font-normal mb-5">
            ...
            <div key={cat.id} className="border-3 border-black p-4 bg-white shadow-[4px_4px_0px_0px_#000] relative">
              ...
              <div className="flex justify-between items-center mb-3">
                <span className="text-sm font-normal text-black">
                  📌 {index + 1}. {cat.label}
                  {cat.isDefault && <span className="text-xs font-normal bg-[#F4F4F1] border border-zinc-300 text-zinc-650 px-1.5 py-0.5 rounded-none ml-2">시스템 기본 탑재</span>}
                </span>
                {!cat.isDefault && (
                  <button
                    type="button"
                    onClick={() => handleDeleteCategory(cat.id, cat.label)}
                    className="text-xs text-zinc-550 border border-black hover:bg-rose-100 hover:text-rose-600 bg-white px-2 py-0.5 font-normal transition flex items-center gap-0.5 cursor-pointer"
                  >
              ...
              <div className="flex flex-wrap gap-2.5 mb-4 border-b border-zinc-200 pb-3.5">
                {cat.options && cat.options.length > 0 ? (
                  cat.options.map((opt) => (
                    <span 
                      key={opt.value} 
                      className="inline-flex items-center gap-1 text-xs font-normal bg-[#F4F4F1] text-black py-1 px-2.5 transition"
                    >
                      ...
                      <button
                        ...
                        className="text-[#FF4D00] hover:text-red-700 font-normal ml-1.5 text-xs outline-none cursor-pointer focus:scale-110 shrink-0"
                        title="이 분류 삭제"
                      >
                        ×
                      </button>
                  ...
                  <span className="text-xs text-zinc-500 font-normal italic py-1">
              ...
              <div className="space-y-2 bg-[#F4F4F1] p-3 border-2 border-black">
                <span className="text-xs font-normal text-black block">💡 이 카테고리 안에 세부 하위 선택지 추가하기</span>
                <div className="flex flex-col sm:flex-row gap-2">
                  <input
                    type="text"
      ...
      <div className="bg-white border-4 border-black p-5 shadow-[6px_6px_0px_0px_#000]">
        <h4 className="text-sm font-black text-black mb-3 flex items-center gap-2 uppercase">
          <Database className="w-4 h-4 text-[#FF4D00] stroke-[2]" />
          JSON 백업 및 복원
        </h4>
        <p className="text-sm text-[#1A1A1A]/85 mb-4 leading-relaxed font-normal">
        ...
        <div className="flex flex-col sm:flex-row gap-3.5">
          ...
          <button
            ...
            className="flex-1 flex items-center justify-center gap-1.5 bg-yellow-300 hover:bg-yellow-400 text-black py-3 font-normal text-sm border-3 border-black shadow-[3px_3px_0px_0px_#000] active:scale-95 transition cursor-pointer"
          >
            <Download className="w-4 h-4 stroke-[2]" /> JSON 백업 파일 다운로드 (.json) 📥
          </button>
          ...
          <button
            ...
            className="flex-1 flex items-center justify-center gap-1.5 bg-black hover:bg-zinc-800 text-white py-3 font-normal text-sm border-3 border-black shadow-[3px_3px_0px_0px_#FF4D00] active:scale-95 transition cursor-pointer"
          >
            <Upload className="w-4 h-4 stroke-[2]" /> JSON 백업 파일로 복원하기 📦
          </button>
        </div>
      </div>
      ...
      <div className="bg-white border-4 border-black p-5 border-rose-500 shadow-[6px_6px_0px_0px_#000]">
        ...
        <div className="space-y-3.5 font-normal text-black">
          ...
          <div className="flex flex-col gap-3">
            ...
            <button
              ...
              className="w-full inline-flex items-center justify-center gap-1.5 bg-rose-200 hover:bg-rose-300 border-3 border-black text-rose-950 py-3 text-sm font-normal shadow-[3px_3px_0px_0px_#000] active:scale-95 transition cursor-pointer"
            >
        ...
        <div className="fixed inset-0 bg-black/75 backdrop-blur-xs flex items-center justify-center p-4 z-50 animate-fade-in">
          <div className="bg-white border-4 border-black p-6 max-w-md w-full shadow-[8px_8px_0px_0px_#FF4D00] space-y-5 relative text-black">
            <div className="flex items-center justify-between border-b-2 border-black pb-3">
              <h4 className="text-sm font-normal uppercase text-rose-600 flex items-center gap-1.5">
            ...
            <div className="space-y-3">
              <p className="text-sm font-normal text-zinc-800 leading-relaxed">
                이 작업은 <span className="text-rose-600 font-normal underline">절대로 되돌릴 수 없는 영구 소멸</span> 작업입니다. 
                현재까지 쌓아온 소중한 미뤄둔 일 진행 상태, 행동 개시 로그, 개인 설정 태그 등이 아낌없이 날아가 버립니다.
              </p>
              <p className="text-sm font-normal text-black bg-rose-50 p-3 border-2 border-dashed border-rose-400">
                정말 데이터 세팅을 완전히 파괴하고 초기화하시겠습니까? 
                동의하고 진행하시려면 아래의 입력란에 정확히 <span className="text-rose-600 font-normal underline">해야지</span> 라고 입력해 주십시오.
              </p>
            </div>

            <div className="space-y-1.5">
              <label className="block text-xs font-normal text-zinc-700">인증 텍스트 입력</label>
              <input
                type="text"
                value={resetConfirmText}
                onChange={(e) => setResetConfirmText(e.target.value)}
                placeholder="해야지"
                className="w-full text-center text-sm font-normal border-3 border-black bg-[#F4F4F1] p-2.5 shadow-[2px_2px_0px_0px_#000] focus:ring-0 focus:outline-none"
              />
            </div>
            ...
            <div className="flex flex-col gap-2 pt-1.5">
              ...
              <button
                ...
                className={`w-full inline-flex items-center justify-center gap-1.5 py-3 text-sm font-normal border-3 border-black shadow-[3px_3px_0px_0px_#000] transition active:scale-95 ${
              ...
              <button
                ...
                className="w-full bg-white hover:bg-zinc-100 text-zinc-700 border-2 border-zinc-400 py-3 text-sm font-normal shadow-[3px_3px_0px_0px_#6b7280] active:scale-[0.98] transition cursor-pointer"
              >
                ❌ 취소하고 돌아가기
              </button>
            </div>
      ...
      {showDeleteOptionModal && deleteOptionTarget && (
        <div className="fixed inset-0 bg-black/65 backdrop-blur-xs flex items-center justify-center p-4 z-50 animate-fade-in">
          <div className="bg-white border-4 border-black p-6 max-w-md w-full shadow-[8px_8px_0px_0px_#000] space-y-5 relative text-black">
            <div className="flex items-center justify-between border-b-2 border-black pb-3">
              <h4 className="text-sm font-normal uppercase text-[#FF4D00] flex items-center gap-1.5">
                <Tag className="w-5 h-5 text-[#FF4D00] stroke-[2]" />
                태그 분류 삭제 경고
              </h4>
            </div>
            ...
            <div className="flex flex-col gap-2 pt-1">
              ...
              <button
                ...
                className="w-full inline-flex items-center justify-center gap-1.5 bg-rose-500 hover:bg-rose-600 text-white py-3 text-sm font-normal border-3 border-black shadow-[3px_3px_0px_0px_#000] active:scale-95 transition cursor-pointer"
              >
                삭제하기
              </button>
      ...
      {showRestoreConfirmModal && (
        <div className="fixed inset-0 bg-black/65 backdrop-blur-xs flex items-center justify-center p-4 z-50 animate-fade-in">
          <div className="bg-white border-4 border-black p-6 max-w-md w-full shadow-[8px_8px_0px_0px_#FF4D00] space-y-5 relative text-black">
            <div className="flex items-center justify-between border-b-2 border-black pb-3">
              <h4 className="text-sm font-normal uppercase text-amber-600 flex items-center gap-1.5">
                <ShieldAlert className="w-5 h-5 stroke-[2]" />
                ⚠️ 백업 데이터 복원 주의
              </h4>
            </div>
      ...
      {/* SETTINGS-WIDE CUSTOM POPUP ALERT/CONFIRM DIALOG OVERLAY */}
      {modalDialog && (
        <div className="fixed inset-0 bg-black/75 backdrop-blur-xs flex items-center justify-center p-4 z-[999] animate-fade-in">
          <div className="bg-white border-4 border-black p-5 max-w-sm w-full shadow-[8px_8px_0px_0px_#000] space-y-4 text-black">
            ...
            <div className="text-xs text-zinc-850 leading-relaxed font-normal whitespace-pre-line">
              {modalDialog.message}
            </div>
            ...
            <div className="flex gap-2.5 pt-1.5 justify-end text-xs">
              ...
              <button
                ...
                className="px-4 py-2 border-2 border-black font-bold text-black bg-[#a7f3d0] hover:bg-emerald-350 shadow-[2px_2px_0px_0px_#000] active:scale-95 transition cursor-pointer flex-1 text-center"
              >
                확인
              </button>
            </div>
